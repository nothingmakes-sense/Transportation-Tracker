...
export default async function LoginPage({
	...
	// ← This is the fix: searchParams must be a Promise
	searchParams: Promise<{ error?: string }>;
}) {
	// Await the promise (required in Next.js 14.2+ / 15+)
	const { error } = await searchParams;

	// Use explicit getSession everywhere for consistency
		...
		<main className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c] text-white">
			<div className="container flex flex-col items-center justify-center gap-12 px-4 py-16">
				...
				<div className="flex w-full max-w-md flex-col gap-8">
					...
					{/* GitHub OAuth (server action - works perfectly) */}
					{/* <form>
						<button
					...
					{/* Email + Password - now client-side (fixes cookie issue) */}
					<SignInForm />
				</div>
				...
				<Link
					className="py-1 text-sm text-white/50 transition hover:text-white"
					href="/"
				>
					← Back to home
				</Link>
